...
def cleanup_existing_processes():
    ...
    try:
        ...
        # Find and kill other chat processes (not this one)
        for script_name in ['qwen3_chat.py', 'gpt_oss_chat.py']:
...
def batch_chat(model_type="qwen3"):
    ...
    if model_type.lower() == "gpt":
        ...
        chat_function = chat_with_gpt_oss
        model_name = "gpt_oss_20b"
        print("🚀 Starting GPT OSS 20B batch processing...\n")
    else:  # default to qwen3
        model, tokenizer = load_qwen3_model()
        chat_function = chat_with_qwen3
        model_name = "qwen3"
        print("🚀 Starting Qwen3 batch processing...\n")
    ...
    jsonl_file = os.path.join(generated_dir, f"{model_name}_batch_qa_results_{timestamp}.jsonl")
    ...
    for i, qa_pair in enumerate(qa_pairs, 1):
        ...
        thinking, response, thinking_token_probs, response_token_probs = chat_function(
    ...
    print(f"✅ {model_name.upper()} batch processing completed! Results saved to: {jsonl_file}")

def chat(model_type="qwen3"):
    # Clean up any existing processes first
    cleanup_existing_processes()
    ...
    os.makedirs(generated_dir, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    jsonl_file = os.path.join(generated_dir, f"{model_name}_chat_history_{timestamp}.jsonl")
    ...
    while True:
        ...
        # Time the generation
        start_time = time.time()
        thinking, response, thinking_token_probs, response_token_probs = chat_function(messages, model, tokenizer, max_new_tokens=512)
        generation_time = time.time() - start_time
        
        print(f"{model_display} (thinking): {thinking}")
        print(f"{model_display}: {response}")
...
if __name__ == "__main__":
    ...
    # Parse command line arguments for model selection only
    model_type = "qwen3"  # default
    
    for arg in sys.argv[1:]:
        if arg.lower() in ["--gpt", "--gpt-oss", "--gpt_oss"]:
            model_type = "gpt"
        elif arg.lower() in ["--qwen", "--qwen3"]:
            model_type = "qwen3"
        elif arg.lower() in ["--help", "-h"]:
            print("Usage: python qwen3_chat.py [MODEL]")
            print("\nModel options:")
            print("  --qwen, --qwen3     Use Qwen3 model (default)")
            print("  --gpt, --gpt-oss    Use GPT OSS 20B model")
            print("\nExamples:")
            print("  python qwen3_chat.py           # Qwen3 (default)")
            print("  python qwen3_chat.py --qwen    # Qwen3 explicitly")
            print("  python qwen3_chat.py --gpt     # GPT OSS 20B")
            exit(0)
    
    print(f"🤖 Using model: {model_type.upper()}")
    print()
    
    # Always run in batch mode (process CSV)
    batch_chat(model_type)
